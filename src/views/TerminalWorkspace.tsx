...
import GitBadge from "../components/workspace/GitBadge";
...
export function TerminalWorkspace({ onLeave }: TerminalWorkspaceProps) {
  ...
  const mode         = activeTab?.mode ?? "claude";
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden">
      <header className="rt-toolbar relative z-[60] flex items-center gap-2 px-3 py-2">
        ...
        <Icon
          name={mode === "claude" ? "claudeLogo" : "layoutGrid"}
          size={15}
          className="rt-accent-text shrink-0"
        />
        <span className="min-w-0 shrink truncate text-sm font-medium" title={cwd ?? undefined}>
          {title}
        </span>
        <GitBadge cwd={cwd} />

        {/* Workspace tools live in the activity rail; global actions stay here. */}
          ...
          {mode === "canvas" && (
            <>
              ...
              >
                ...
                {showLabels && <span>Reset canvas</span>}
              </button>
